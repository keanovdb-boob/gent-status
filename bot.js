...
exports.start = function(SETUP) {
  ...
  const UpdateEmbed = function() {
    let dot = TICK_N % 2 === 0 ? 'Gent  ' : 'Roleplay';
    let embed = new Discord.RichEmbed()
    .setTitle(`:battery: | Gent  Roleplay Status`)
    .setColor("@0c0d0c")
    .setFooter(TICK_N % 2 === 0 ? '⚪ Gent   RP' : '⚫ Gent   RP')
    .setTimestamp(new Date())
    .addField('\n\u200b\nHoe kan je de server joinen?','Je kan de server joinen doormiddel van **SOON** in te voeren bij Direct Connect. Onderaan staat de server status om te kijken hoeveel mensen er online zijn en in de wachtrij staan.',false)
    if (STATUS !== undefined)
    {
  ...
  bot.on('ready',() => {
    ...
    log(LOG_LEVELS.INFO,'Started...');   
      bot.user.setActivity(`${players.length}/${MAX_PLAYERS} spelers!`, { type: 'WATCHING' }); 
